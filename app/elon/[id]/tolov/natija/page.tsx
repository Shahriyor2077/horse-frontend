...
function PaymentResultContent() {
    ...
    // Reactivation (no packageType) → show pending tab via ?success=true
    useEffect(() => {
        if (status !== 'completed') return;
        setCountdown(3);
        const isReactivation = payment && !payment.packageType;
        const redirectUrl = isReactivation ? '/profil/elonlarim?success=true' : '/profil/elonlarim';
                    ...
                    return (
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 text-center shadow-sm">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 mb-2">
                                {isReactivation
                                    ? "E'loningiz moderatsiyaga yuborildi. Tekshiruvdan o'tgach faollashadi."
                                ...
                                <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 mb-6 text-left space-y-2">
                                    ...
                                    {isReactivation && (
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-medium text-slate-900 dark:text-slate-100">Faollashtirish</span>
                                        </div>
                                    )}
                                    <div className="flex justify-between text-sm">
